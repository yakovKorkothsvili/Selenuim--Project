...
//	}
	...
	private static void fillDetailsToThenConnectTofacebook() throws InterruptedException {
		...
		try {
			driver.findElement(By.xpath(
					"//span[@class='a8c37x1j ni8dbmo4 stjgntxs l9j0dhe7 ltmttdrg g0qnabr5 ojkyduve' and contains(.,'המשך')]"))
					.click();
	...
	private static void switchBackToMainTabToTakeNameAndEmailItsInAIfame() throws InterruptedException {
		...
		for (int i = 0; i < iframe1.size(); i++) {
			try {
				driver.switchTo().frame(i);
				nameEmail = driver.findElements(
						By.xpath("//div[@class='styles__UserProfileDetailsContainer-sc-1jj4i2k-0 kSRUDj']//div"));
...
//	}
	...
	private static void typeingLupaInSearchBarAndTakingTheResults() throws InterruptedException {
		...
		for (int i = 0; i < iframe.size(); i++) {
			...
			try {
				driver.switchTo().frame(i);
				// click on return button so u can open the serach bar
